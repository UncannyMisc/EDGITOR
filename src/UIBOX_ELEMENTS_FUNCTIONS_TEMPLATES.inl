...
template<typename T>
void uibox_element_add_button(UIBOX_INFO* uibox, uint16_t x, uint16_t y, uint16_t w, uint16_t h, std::string text, std::string sel_text, T* input_var, T button_var, COLOR col, bool hold)
{
	std::shared_ptr<UIBOX_ELEMENT_BUTTON<T>> _element = std::make_shared<UIBOX_ELEMENT_BUTTON<T>>();
	_element->x = x;
	_element->y = y;
